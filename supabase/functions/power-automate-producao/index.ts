...
function dateExcelSerial(value: unknown) {
  const iso = dateIso(value);
  if (!iso) return "";
  const [year, month, day] = iso.split("-").map(Number);
  const excelEpoch = Date.UTC(1899, 11, 30);
  return Math.round((Date.UTC(year, month - 1, day) - excelEpoch) / 86_400_000);
}

function ruptureDateExcelSerial(row: DatabaseRow, days: number) {
  return dateExcelSerial(addDaysIso(row.data_fabricacao, days));
}
...
function excelRow(row: DatabaseRow, tests: DatabaseRow[]): JsonCell[] {
  // Envia datas como números seriais do Excel. Assim, o Office Script não
  // precisa interpretar texto com dia e mês ambíguos; a planilha apenas aplica
  // o formato visual dd-mm-aaaa (DD-MM-YYYY).
  const cure14 = dateIso(row.cura_14);
  const cure28 = dateIso(row.cura_28);
  return [
    text(row.pista),
    text(row.pedido),
    text(row.lote),
    ...
    dateExcelSerial(row.data_fabricacao),
    dateExcelSerial(cure14),
    dateExcelSerial(cure28),
    ...
    text(row.despro_fim),
    text(row.tempo_cura),
    ruptureDateExcelSerial(row, 7),
    dateExcelSerial(cure14) || ruptureDateExcelSerial(row, 14),
    dateExcelSerial(cure14) || ruptureDateExcelSerial(row, 14),
    dateExcelSerial(cure28) || ruptureDateExcelSerial(row, 28),
    dateExcelSerial(cure28) || ruptureDateExcelSerial(row, 28),
